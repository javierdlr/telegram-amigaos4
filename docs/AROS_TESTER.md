...
reply and `q`/`quit` to quit. Inside chat mode, type normal text to send. It auto-reads every 5 seconds by default while waiting for input; use `/watch <seconds>` to change the interval, `/watch off` to disable it, or `/read`, `/list`, `/last`, `/status`, `/back` and `/quit`. It does not send replies automatically.
