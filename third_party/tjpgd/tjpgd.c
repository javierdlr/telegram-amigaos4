...
static const uint16_t Ipsf[64] = {	/* See also aa_idct.png */
	(uint16_t)8192, (uint16_t)11362, (uint16_t)10703, (uint16_t)9632, (uint16_t)8192, (uint16_t)6436, (uint16_t)4433, (uint16_t)2260,
	(uint16_t)11362, (uint16_t)15760, (uint16_t)14846, (uint16_t)13361, (uint16_t)11362, (uint16_t)8927, (uint16_t)6149, (uint16_t)3134,
	(uint16_t)10703, (uint16_t)14846, (uint16_t)13984, (uint16_t)12585, (uint16_t)10703, (uint16_t)8409, (uint16_t)5792, (uint16_t)2953,
	(uint16_t)9632, (uint16_t)13361, (uint16_t)12585, (uint16_t)11326, (uint16_t)9632, (uint16_t)7568, (uint16_t)5213, (uint16_t)2657,
	(uint16_t)8192, (uint16_t)11362, (uint16_t)10703, (uint16_t)9632, (uint16_t)8192, (uint16_t)6436, (uint16_t)4433, (uint16_t)2260,
	(uint16_t)6436, (uint16_t)8927, (uint16_t)8409, (uint16_t)7568, (uint16_t)6436, (uint16_t)5057, (uint16_t)3483, (uint16_t)1775,
	(uint16_t)4433, (uint16_t)6149, (uint16_t)5792, (uint16_t)5213, (uint16_t)4433, (uint16_t)3483, (uint16_t)2399, (uint16_t)1223,
	(uint16_t)2260, (uint16_t)3134, (uint16_t)2953, (uint16_t)2657, (uint16_t)2260, (uint16_t)1775, (uint16_t)1223, (uint16_t)623
};
...
{
	const int CVACC = 1024; /* vendored: 32-bit int on every lane; color factors pre-folded to integers (no soft-float on m68k -O0) */	/* Adaptive accuracy for both 16-/32-bit systems */
		...
		if (JD_FORMAT != 2) {	/* RGB output (build an RGB MCU from Y/C component) */
			for (iy = 0; iy < my; iy++) {
				...
				for (ix = 0; ix < mx; ix++) {
					...
					*pix++ = /*R*/ BYTECLIP(yy + ((int)1435 * cr) / CVACC);
					*pix++ = /*G*/ BYTECLIP(yy - ((int)352 * cb + (int)731 * cr) / CVACC);
					*pix++ = /*B*/ BYTECLIP(yy + ((int)1814 * cb) / CVACC);
				}
			}
...
JRESULT jd_decomp_begin (
	JDEC* jd,
	uint8_t scale
)
{
	if (!jd || scale > (JD_USE_SCALE ? 3 : 0)) return JDR_PAR;
	jd->scale = scale;
	jd->dcv[2] = jd->dcv[1] = jd->dcv[0] = 0;	/* Initialize DC values */
	jd->decomp_x = jd->decomp_y = 0;
	jd->decomp_rst = jd->decomp_rsc = 0;
	jd->decomp_active = 1;
	jd->decomp_done = 0;
	return JDR_OK;
}
...
{
	unsigned int mx, my, count;
	JRESULT rc;

	if (!jd || !outfunc || !done || !max_mcus) return JDR_PAR;
	if (jd->decomp_done) {
		*done = 1;
		return JDR_OK;
	}
	if (!jd->decomp_active) return JDR_PAR;
	mx = jd->msx * 8;
	my = jd->msy * 8;
	count = 0;
	while (count < max_mcus && jd->decomp_y < jd->height) {
		if (jd->nrst && jd->decomp_rst++ == jd->nrst) {
			rc = restart(jd, jd->decomp_rsc++);
			if (rc != JDR_OK) {
				jd->decomp_active = 0;
				return rc;
			}
		...
		rc = mcu_output(jd, outfunc, (unsigned int)jd->decomp_x,
		                (unsigned int)jd->decomp_y);
		if (rc != JDR_OK) {
			jd->decomp_active = 0;
			return rc;
		}
		jd->decomp_x += mx;
		if (jd->decomp_x >= jd->width) {
			jd->decomp_x = 0;
			jd->decomp_y += my;
		}
		++count;
	}
	if (jd->decomp_y >= jd->height) {
		jd->decomp_active = 0;
		jd->decomp_done = 1;
	}
	*done = jd->decomp_done ? 1 : 0;
	return JDR_OK;
}
...
)
{
	...
	rc = jd_decomp_begin(jd, scale);
	if (rc != JDR_OK) return rc;
	done = 0;
	while (!done) {
		rc = jd_decomp_step(jd, outfunc, ~0U, &done);
		if (rc != JDR_OK) return rc;
	}
	return JDR_OK;
}
